...
export async function POST(request: NextRequest) {
  ...
  const { data: proof, error: proofErr } = await supabaseServer
    ...
    .select('object_path, mime_type, original_filename, notes, size_bytes, created_at')
  ...
  if (signedErr || !signed?.signedUrl) {
    return NextResponse.json({ ok: false, message: signedErr?.message ?? 'No se pudo firmar URL' }, { status: 400 })
  }
