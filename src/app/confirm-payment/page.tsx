...
function isApproved(r: PayphoneConfirmResponse): boolean {
  const ts = normalizeStatus(r.transactionStatus ?? '')
  const sc = Number(r.statusCode ?? 0)
  return ts === 'approved' || ts === 'success' || ts === 'paid' || sc === 3
}

function isRejected(r: PayphoneConfirmResponse): boolean {
  const ts = normalizeStatus(r.transactionStatus ?? '')
  const sc = Number(r.statusCode ?? 0)
  return (
  ...
  async function verifyOnce() {
    ...
    try {
      ...
      if (!confirm.ok || !confirm.json) {
        setStatus('failed')
        setMessage(
          `PayPhone respondió ${confirm.status}\n\n${(confirm.raw ?? '').slice(0, 1000)}…`
        )
        setIsPolling(false)
        return
      }

      // (opcional) si PayPhone te devuelve un link al checkout, aquí lo asignas
      setCheckoutUrl(null)
      ...
      setStatus('failed')
      setMessage(
        e instanceof Error ? e.message : 'No se pudo verificar el pago.'
      )
      setIsPolling(false)
    }
  }
